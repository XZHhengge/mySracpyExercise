...
def get_url(code=0,ips=[]):
    ...
    try:
        ...
        url = 'http://hlzjc.gdcyl.org/tp/votetparr.asp'
        # ArticleId=120770&ClassID=65&SchoolID=22
        formdata = {'ArticleId': 120770, 'ClassID': 65, 'SchoolID': 22}

        resopnse = requests.post(url=url, headers=headers2, data=formdata, proxies=proxies)
        ...
        print(u"第%s次 [%s] [%s]：投票%s (剩余可用代理IP数：%s)" % (code,date,ip, resopnse.text,len(ips)))
ips = []
