...
# class UserOperation(wx.Dialog):
    ...
    def ClickButton(self, event):
        ...
        if source_id == 10:
            print("查询操作！")
            inquire_button = InquireOp(None, title="电力公司收费管理系统管理系统", size=(1024, 668))
            inquire_button.Show()
            self.Close(True)
        elif source_id == 11:
            print("添加操作！")
            add_button = AddOp(None, title="电力公司收费管理系统管理系统", size=(1024, 668))
            add_button.Show()
            self.Close(True)
        elif source_id == 12:
            print("删除操作！")
            del_button = DelOp(None, title="电力公司收费管理系统管理系统", size=(1024, 668))
            del_button.Show()
            self.Close(True)
        elif source_id == 13:
            self.Close(True)
...
class AddOp(UserOperation):
    def __init__(self, *args, **kw):
        ...
        #################################################################################
        # 创建静态框
        sb_name = wx.StaticBox(self.pnl, label="客户名")
